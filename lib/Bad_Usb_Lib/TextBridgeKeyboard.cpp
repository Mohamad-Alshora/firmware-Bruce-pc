...
void TextBridgeKeyboard::begin(const uint8_t *layout) { _asciimap = layout; }

void TextBridgeKeyboard::end(void) { _stream = nullptr; }

int TextBridgeKeyboard::getReportData(uint8_t *buffer, size_t size) { return 0; }
...
bool TextBridgeKeyboard::isModifierKey(uint8_t keyCode) {
    return (
        keyCode == KEY_LEFT_CTRL || keyCode == KEY_LEFT_SHIFT || keyCode == KEY_LEFT_ALT ||
        keyCode == KEY_LEFT_GUI || keyCode == KEY_RIGHT_CTRL || keyCode == KEY_RIGHT_SHIFT ||
...
size_t TextBridgeKeyboard::println(void) {
    ...
    if (_stream) { sendKeyCommand("ENTER"); }
    return 1;
}
